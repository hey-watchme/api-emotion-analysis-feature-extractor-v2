...
| └ EC2配置場所 | `/home/ubuntu/emotion-analysis-feature-extractor` | 設定ファイル・.env配置先 |
| | | |
| **🔗 呼び出し元** | | |
...
### ✅ 命名統一・デプロイ問題修正完了（2025-11-18）

**2025-11-18: デプロイ問題の根本修正**
- **問題**: 古いコンテナ（v3）が稼働し続ける
- **原因**: `docker system prune -a -f` + `docker-compose pull` の組み合わせによるキャッシュ問題
- **解決**: 強制的な `docker pull` + ECRリポジトリ・コンテナ名の統一
- **検証**: `verify-deployment.sh` スクリプト追加

**2025-10-29: 統一命名規則への対応完了**
- **Nginxエンドポイント**: `/emotion-features/` → `/emotion-analysis/feature-extractor/`
- **ECRリポジトリ**: `watchme-emotion-analysis-feature-extractor-v3` → `watchme-emotion-analysis-feature-extractor`
- **コンテナ名**: `emotion-analysis-feature-extractor-v3` → `emotion-analysis-feature-extractor`
- **統一原則**: `/{domain}/{service}/` に準拠
